...
def DBConnect(dbName=None):
    ...
    """
    conn = mysql.connect(host='localhost', user='root', password='12Nhom@&37',
...
def preprocess_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    cols_2_drop = ['timestamp', 'sentiment', 'possibly_sensitive', 'original_text']
    try:
...
if __name__ == "__main__":
    createDB(dbName='tweets_data')
    emojiDB(dbName='tweets_data')
    createTables(dbName='tweets_data')
